...
def Co2densidad(df):
    ...
    df["densidad"] = np.where(df["profundidad"] > 2, 700, px(df["profundidad"]))
    return round(df["densidad"],2)
def bachuprob(df):
    df["densidad"]= Co2densidad(df)
    df["Bo_Norm"]= Bo()
    df["capacidad"]= df["densidad"]*((df["rf"]) * df["ooip"]) / (df["Bo_Norm"] * 10000000000)
    return round(df,2)
def zhoca(df):
    ...
    df["densidad"] = Co2densidad(df)
    df["Bo_Norm"] = Bo()
    df["Capacidad"]= df["ooip"] * df["densidad"] * df["Bo_Norm"] * df["E"]/1000000
    return round(df,2)
